...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form wire:submit="create" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-5 grid grid-cols-2 sm:grid-cols-5 gap-3 items-end mb-6">
        <div>
            <x-input-label for="code" :value="__('Code')" />
            <x-text-input wire:model="code" id="code" class="mt-1 w-full text-sm" />
            @error('code') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>
        <div>
            <x-input-label for="type" :value="__('Type')" />
            <select wire:model="type" id="type" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 text-sm">
        ...
        <div>
            ...
            <x-text-input wire:model="value" id="value" type="number" step="0.01" class="mt-1 w-full text-sm" />
            @error('value') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>
        <div>
            <x-input-label for="max_uses" :value="__('Utilisations max')" />
            <x-text-input wire:model="max_uses" id="max_uses" type="number" class="mt-1 w-full text-sm" />
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm divide-y divide-gray-100 dark:divide-gray-700">
        @forelse ($codes as $promo)
            <div class="flex items-center gap-4 p-4" wire:key="promo-{{ $promo->id }}">
                <div class="flex-1">
                    <p class="font-mono font-medium text-gray-900 dark:text-gray-100">{{ $promo->code }}</p>
                ...
                <span @class(['text-xs px-2 py-1 rounded-full', 'bg-green-100 text-green-800' => $promo->active, 'bg-gray-100 text-gray-500' => ! $promo->active])>
                    {{ $promo->active ? __('Actif') : __('Inactif') }}
                </span>
                <button type="button" wire:click="toggleActive({{ $promo->id }})" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
